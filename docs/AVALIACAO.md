...
| **U2** | Prova (20%) + Sprint 2 (30%) + Sprint 3 (30%) + Quizzes 1 e 3 (10% cada) | Individual + Grupo + Individual |
| **U3** | **Prova (50%)** + Vídeo Final (30%) + Proposta de Melhoria (20%) | Individual + Grupo + Grupo |

> O **Quiz de Reposição** não cria um novo componente nem altera estes pesos: sua nota apenas **substitui a menor nota** entre os Quizzes 1, 2 e 3 (ver seção *Quizzes*).
...
- **Duração**: **40 minutos** de atividade após o início (dentro da janela 10:30–12:00)
...
## Quizzes (Individuais — Multiprova)

Os quizzes são individuais, aplicados no **Multiprova**, com **janela de 1 dia** para realização e **25 minutos** de atividade após iniciados.

| Quiz | Conteúdo | Aplicação | Peso |
|------|----------|-----------|------|
| Quiz 1 | Modelos de ciclo de vida + Scrum + Manifesto Ágil | 06/05 ✓ | 10% da U2 |
| Quiz 2 | XP + Lean + Kanban | 16/04 ✓ | 10% da U1 |
| Quiz 3 | DevOps + CI/CD + Docker | 12/05 ✓ | 10% da U2 |
| **Quiz de Reposição** | Conteúdo dos Quizzes 1, 2 e 3 | **a definir** (avisado com antecedência) | substitui a menor nota |

### Quiz de Reposição

- **Formato**: idêntico aos demais — no Multiprova, **janela de 1 dia** para iniciar e **25 minutos** de duração após o início.
- **Conteúdo**: abrange tudo o que foi cobrado nos Quizzes 1, 2 e 3 (modelos de ciclo de vida, Scrum, Manifesto Ágil, XP, Lean, Kanban, DevOps, CI/CD, Docker).
- **Efeito na nota**: a nota obtida **substitui a menor** das notas entre os Quizzes 1, 2 e 3. Como esses quizzes pesam em unidades diferentes (Quiz 2 em U1; Quizzes 1 e 3 em U2), a substituição recai sobre a unidade do quiz de menor nota.
- **Não** cria um novo componente de nota nem altera os pesos das unidades (que continuam somando 100%).
- **Data**: a ser definida; será anunciada com antecedência pelos canais da disciplina.
...
### Avaliação Individual (Prova e Quizzes)

A prova e os quizzes são **estritamente individuais**. Na prova, a consulta é permitida apenas à folha A4 manuscrita.
...
- Na **prova** e nos **quizzes**, o uso de IA não é permitido
...
**P: Como funciona o Quiz de Reposição?**  
R: Mesmo formato dos demais (Multiprova, janela de 1 dia, 25 min após iniciar). Cobre o conteúdo dos Quizzes 1, 2 e 3, e a nota obtida **substitui a menor** entre eles. A data será avisada com antecedência.
...
**P: Quais quizzes contam na avaliação?**  
R: Os Quizzes 1, 2 e 3 (mais o Quiz de Reposição, que substitui a menor nota entre eles). Os quizzes 6 e 7 foram cancelados na reorganização de 25/05.
